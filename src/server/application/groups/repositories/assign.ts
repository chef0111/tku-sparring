...
export type GroupAssignContext = {
  id: string;
  tournamentId: string;
...
export type GroupAssignmentStore = {
  findGroup: (groupId: string) => Promise<GroupAssignContext | null>;
  ...
  ) => Promise<{ assigned: number; groupsRun: number; groupsSkipped: number }>;
